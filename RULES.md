...
If on a square next to the throne, the attackers must occupy the three remaining squares around him and be the one to move.
...
When the defenders don’t have enough pieces left to make an exit fort, the corners are blocked, and the attackers can move without unblocking a corner.
...
When only three spaces are left on the side and the defender only has five pieces plus the king left.
...
When the attackers have all corners blocked and only two spaces left on any side.
...
It does not count as the side being blocked if the side spaces are not connected
